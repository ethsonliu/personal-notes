...
- [其它编程建议](#其它编程建议)
  ...
  - [指针和引用符号放在左边还是右边](#指针和引用符号放在左边还是右边)
- [参考](#参考)
...
在`.cpp`文件中定义一个不需要被外部引用的变量时，可以将它们放在匿名命名空间或声明为`static`（个人推荐用 static，至少视觉上 static 更直观）。但是不要在`.h`文件中这么做。
...
1. Javadoc `/**  */`风格，具体参考 jdk。
2. 双斜杠 `//` 风格。

以下的例子都是基于上层开发，注释只是为了方便阅读代码，而不是生成文档。
...
如果是我个人的项目，风格统一放在左边，例如 `int* p = nullptr`、`int& p = a`。
