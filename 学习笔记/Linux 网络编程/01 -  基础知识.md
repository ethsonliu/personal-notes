...
- **首部长度（IHL, Internet Header Length）**
    ...
    4 位，IP 报头的长度，即固定部分的长度（20 字节）和可变部分的长度之和。最大为 1111，即 10 进制的 15，代表 IP 报头的最大长度可以为 15 个32bits（4 字节），也就是最长可为 15 * 4 = 60 字节，除去固定部分的长度 20 字节，可变部分的长度最大为 40 字节。一般来说此处的值为 0101，表示头长度为20 字节。
